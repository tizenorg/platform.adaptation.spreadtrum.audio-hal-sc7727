...
{
    audio_return_t audio_ret = AUDIO_RET_OK;
    audio_hal_t *ah = (audio_hal_t *)audio_intf;
    device_info_t *devices = NULL;
...
audio_return_t audio_pcm_open (void *audio_intf, void **pcm_handle, uint32_t direction, void *sample_spec, uint32_t period_size, uint32_t periods)
{
    ...
    AUDIO_RETURN_VAL_IF_FAIL(audio_intf, AUDIO_ERR_PARAMETER);
    ...
    *pcm_handle = __tinyalsa_open_device(ss, (size_t)period_size, (size_t)periods, direction);
    ...
    if ((err = audio_pcm_set_params(audio_intf, *pcm_handle, direction, sample_spec, period_size, periods)) != AUDIO_RET_OK) {
