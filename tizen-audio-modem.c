...
static int __voice_pcm_set_params(audio_hal_t *ah, snd_pcm_t *pcm)
{
    ...
    if(ah->modem.samplerate > 0) {
       err = snd_pcm_hw_params_set_rate(pcm, params, ah->modem.samplerate, 0);
       if (err < 0) {
           AUDIO_LOG_ERROR("snd_pcm_hw_params_set_rate() : failed! - %s\n", snd_strerror(err));
       }
    } else {
       err = snd_pcm_hw_params_set_rate(pcm, params, (ah->device.route_flag & AUDIO_ROUTE_FLAG_NETWORK_WB) ? 16000 : 8000, 0);
       if (err < 0) {
           AUDIO_LOG_ERROR("snd_pcm_hw_params_set_rate() : failed! - %s\n", snd_strerror(err));
       }
    }
...
int _voice_pcm_open(audio_hal_t *ah)
{
    ...
    if ((audio_ret = audio_alsa_pcm_open((void *)am, (void **)&ah->modem.vbc.voice_pcm_handle_p, VOICE_PCM_DEVICE, AUDIO_DIRECTION_OUT, 0)) < 0) {
        AUDIO_LOG_ERROR("snd_pcm_open for %s failed. %x", VOICE_PCM_DEVICE, audio_ret);
        return AUDIO_ERR_IOCTL;
    }
    ret = __voice_pcm_set_params(ah, ah->modem.vbc.voice_pcm_handle_p);
    ...
    if ((audio_ret = audio_alsa_pcm_open((void *)am, (void **)&ah->modem.vbc.voice_pcm_handle_c, VOICE_PCM_DEVICE, AUDIO_DIRECTION_IN, 0)) < 0) {
        AUDIO_LOG_ERROR("snd_pcm_open for %s failed. %x", VOICE_PCM_DEVICE, audio_ret);
        return AUDIO_ERR_IOCTL;
    }
    ret = __voice_pcm_set_params(ah, ah->modem.vbc.voice_pcm_handle_c);
...
int _voice_pcm_close (audio_hal_t *ah,int reset)
{
    AUDIO_LOG_INFO("close voice pcm handles");

    if (ah->modem.vbc.voice_pcm_handle_p) {
        audio_alsa_pcm_close((void *)am, ah->modem.vbc.voice_pcm_handle_p);
        ah->modem.vbc.voice_pcm_handle_p = NULL;
        AUDIO_LOG_INFO("pcm playback device close");
    }

    if (ah->modem.vbc.voice_pcm_handle_c) {
        audio_alsa_pcm_close((void *)am, ah->modem.vbc.voice_pcm_handle_c);
        ...
        if (!memcmp(&read_head.tag[0], VBC_CMD_TAG, 3)) {
            switch (read_head.cmd_type) {
                case VBC_CMD_PCM_OPEN: {
                    ...
                    //_do_route_voicecall(ah, &(ah->device.last_device_info), ah->device.last_num_of_devices);
...
//                    /* close all pcm */
//                    if (ah->cb_intf.close_all_devices) {
//                        ah->cb_intf.close_all_devices(ah->platform_data);
//                    }
                    /* close device */
